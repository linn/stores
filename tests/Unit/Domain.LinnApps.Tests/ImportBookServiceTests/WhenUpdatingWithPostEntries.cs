    ...
    public class WhenUpdatingWithPostEntries : ContextBase
    {
        ...
        public void SetUp()
        {
            ...
            var newImportBook = new ImportBook
            {
                ...
                InvoiceDetails = new List<ImportBookInvoiceDetail>(),
                OrderDetails = new List<ImportBookOrderDetail>(),
                PostEntries = new List<ImportBookPostEntry> { updatedFirstPostEntry, secondPostEntry }
            };
        ...
        public void ShouldHaveUpdatedInvoiceDetail()
        {
            this.impbook.PostEntries.FirstOrDefault(
                    ...
                    x.LineNumber == 1 &&
                    x.EntryCodePrefix == "PRE" &&
                    x.EntryCode == "code vert" &&
                    x.EntryDate == this.now.AddDays(-5) &&
                    x.Reference == "refer fence" &&
                    x.Duty == 111 &&
                    x.Vat == 222)
                .Should().NotBeNull();
        }
        ...
        public void ShouldHaveAddedInvoiceDetail()
        {
            ...
            this.impbook.PostEntries.FirstOrDefault(
                    x =>
                        x.ImportBookId == this.impbookId &&
                        x.LineNumber == 2 &&
                        x.EntryCodePrefix == "DL" &&
                        x.EntryCode == "code blanc" &&
                        x.EntryDate == this.now.AddDays(-6) &&
                        x.Reference == "hocus pocus" &&
                        x.Duty == 33 &&
                        x.Vat == 44)
                .Should().NotBeNull();
        }
